...
const PropertySection = () => {
  ...
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    //tab filter ar sathe sathe automatic navigation :
    if (tab === "Villa") navigate('/villa');
    if (tab === "Family House") navigate('/family-house');
    if (tab === "Apartment") navigate('/apartment'); 
  };
          ...
          >
            <div className="relative overflow-hidden rounded-3xl bg-gray-100 h-72">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                <span className="bg-white/90 dark:bg-black/70 backdrop-blur-md px-3 py-1 rounded-lg text-[10px] font-bold uppercase text-black dark:text-white">
                  {item.type}
                </span>
                <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase text-white ${item.status === 'For Rent' ? 'bg-orange-500' : 'bg-green-600'}`}>
                  {item.status}
                </span>
              </div>
